...
"""
==============================================================================
Ly thuyet chuyen mau RGB sang HSI
***
...
##ham chuyen doi khong gian mau RGB sang HSI dung Pillow
def RGBtoHSIusePillow(image):

    ##lay kich thuoc anh
    width = image.size[0]
    height = image.size[1]
    
    ##tao tuple gom 4 phan tu chua anh H S I co cung mode va kich thuoc voi anh goc
    HSI = (Image.new(image.mode, image.size),
    ...
    for x in range (width):
         for y in range (height):
            ...
            HSI[0].putpixel((x, y), (H, H, H))
            HSI[1].putpixel((x, y), (S, S, S))
            HSI[2].putpixel((x, y), (I, I, I))
            HSI[3].putpixel((x, y), (I, S, H))
...
def main():
    ...
    ##lay anh ve
    value = RGBtoHSIusePillow(imgPIL)

    ## chuyen anh PIL ve anh thuong de hien thi
    Hue = np.array(value[0])
    Saturation = np.array(value[1])
    Intensity = np.array(value[2])
    HSIImage = np.array(value[3])

    ## hien thi hinh
    cv2.imshow('Hinh Goc RGB co gai lena', img)
    cv2.imshow('Hinh Hue ', Hue)
    cv2.imshow('Hinh Saturation', Saturation)
    cv2.imshow('Hinh Intensity', Intensity)
    cv2.imshow('Hinh HSI', HSIImage)
